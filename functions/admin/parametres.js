...
import { getNotificationEmail, setNotificationEmail } from '../_shared/settings-kv.js';

function page({ notificationEmail, passwordError, passwordOk, emailError, emailOk }) {
  ...
  if (action === 'password') {
    ...
    return new Response(page({ notificationEmail, passwordOk: true }), {
      headers: {
        ...
        'Set-Cookie': `${COOKIE_NAME}=${encodeURIComponent(newPassword)}; HttpOnly; Secure; SameSite=Lax; Path=/admin; Max-Age=2592000`,
      },
    });
  }
